...
(function () {
  ...
  B.prototype.Contains = function (b) {
    ...
    return (e =
      (e =
        (e =
          (e = e && this.lowerBound.x <= b.lowerBound.x) && this.lowerBound.y <= b.lowerBound.y) &&
        b.upperBound.x <= this.upperBound.x) && b.upperBound.y <= this.upperBound.y);
  };
...
(function () {
  ...
  y.prototype.b2EdgeShape = function (k, z) {
    ...
    this.m_coreV1.Set(
      -F.b2_toiSlop * (this.m_normal.x - this.m_direction.x) + this.m_v1.x,
      -F.b2_toiSlop * (this.m_normal.y - this.m_direction.y) + this.m_v1.y
    );
    this.m_coreV2.Set(
      -F.b2_toiSlop * (this.m_normal.x + this.m_direction.x) + this.m_v2.x,
      -F.b2_toiSlop * (this.m_normal.y + this.m_direction.y) + this.m_v2.y
    );
...
(function () {
  ...
  k.prototype.ApplyImpulse = function (a, c) {
    if (this.m_type == k.b2_dynamicBody) {
      ...
      this.m_angularVelocity +=
        this.m_invI * ((c.x - this.m_sweep.c.x) * a.y - (c.y - this.m_sweep.c.y) * a.x);
    }
  };
  ...
  k.prototype.GetLinearVelocityFromWorldPoint = function (a) {
    return new y(
      this.m_linearVelocity.x - this.m_angularVelocity * (a.y - this.m_sweep.c.y),
      this.m_linearVelocity.y + this.m_angularVelocity * (a.x - this.m_sweep.c.x)
    );
  };
    ...
    for (b = 0; b < this.m_bodyCount; ++b) {
      ...
      if (e.GetType() != k.b2_staticBody) {
        ...
        if (m * m > A.b2_maxRotationSquared)
          e.m_angularVelocity =
            e.m_angularVelocity < 0 ? -A.b2_maxRotation * a.inv_dt : A.b2_maxRotation * a.inv_dt;
    ...
    for (c = 0; c < this.m_bodyCount; ++c) {
      ...
      if (g.GetType() != k.b2_staticBody) {
        ...
        if (e * e > A.b2_maxRotationSquared)
          g.m_angularVelocity =
            g.m_angularVelocity < 0 ? -A.b2_maxRotation * a.inv_dt : A.b2_maxRotation * a.inv_dt;
      ...
      if (a & E.e_shapeBit)
        for (c = this.m_bodyList; c; c = c.m_next) {
          ...
          for (g = c.GetFixtureList(); g; g = g.m_next) {
            ...
            else c.IsAwake() == false ? f.Set(0.6, 0.6, 0.6) : f.Set(0.9, 0.7, 0.7);
  ...
  h.prototype.Solve = function (a) {
    ...
    for (c = this.m_bodyList; c; c = c.m_next)
      c.IsAwake() == false ||
        c.IsActive() == false ||
        (c.GetType() != k.b2_staticBody && c.SynchronizeFixtures());
    this.m_contactManager.FindNewContacts();
  };
  h.prototype.SolveTOI = function (a) {
    ...
    e.Initialize(
      this.m_bodyCount,
      A.b2_maxTOIContactsPerIsland,
      A.b2_maxTOIJointsPerIsland,
      null,
      this.m_contactManager.m_contactListener,
      this.m_contactSolver
    );
    ...
    for (b = this.m_contactList; b; b = b.m_next) b.m_flags &= ~(l.e_toiFlag | l.e_islandFlag);
    for (b = this.m_jointList; b; b = b.m_next) b.m_islandFlag = false;
    for (;;) {
      var m = null,
        r = 1;
      for (b = this.m_contactList; b; b = b.m_next)
        if (!(b.IsSensor() == true || b.IsEnabled() == false || b.IsContinuous() == false)) {
          ...
          if (b.m_flags & l.e_toiFlag) c = b.m_toi;
            ...
            g = g.m_body;
            if (
              (c.GetType() != k.b2_dynamicBody || c.IsAwake() == false) &&
...
(function () {
  ...
  w.prototype.Evaluate = function () {
    ...
    S.CollideCircles(
      this.m_manifold,
      this.m_fixtureA.GetShape() instanceof F ? this.m_fixtureA.GetShape() : null,
      j.m_xf,
      this.m_fixtureB.GetShape() instanceof F ? this.m_fixtureB.GetShape() : null,
      o.m_xf
    );
  };
  ...
  A.prototype.Reset = function (j, o) {
    ...
    if (!j || !o) this.m_fixtureB = this.m_fixtureA = null;
      ...
      var q = j.GetBody(),
        n = o.GetBody();
      if (
        q.GetType() != D.b2_dynamicBody ||
        q.IsBullet() ||
        n.GetType() != D.b2_dynamicBody ||
        n.IsBullet()
      )
  ...
  L.prototype.SolveVelocityConstraints = function () {
    for (
      ...
      if (a.pointCount == 1) {
        ...
        g = j.normalImpulse;
        var ba =
            (C.x - t * o.rB.y - x.x + v * o.rA.y) * b + (C.y + t * o.rB.x - x.y - v * o.rA.x) * e,
          ca =
            (C.x - t * j.rB.y - x.x + v * j.rA.y) * b + (C.y + t * j.rB.x - x.y - v * j.rA.x) * e;
        n = ba - o.velocityBias;
        c = ca - j.velocityBias;
        f = a.K;
  ...
  I.prototype.Evaluate = function () {
    ...
    this.b2CollideEdgeAndCircle(
      this.m_manifold,
      this.m_fixtureA.GetShape() instanceof G ? this.m_fixtureA.GetShape() : null,
      j.m_xf,
      this.m_fixtureB.GetShape() instanceof F ? this.m_fixtureB.GetShape() : null,
      o.m_xf
    );
  ...
  Y.prototype.Evaluate = function () {
    ...
    S.CollidePolygonAndCircle(
      this.m_manifold,
      this.m_fixtureA.GetShape() instanceof K ? this.m_fixtureA.GetShape() : null,
      j.m_xf,
      this.m_fixtureB.GetShape() instanceof F ? this.m_fixtureB.GetShape() : null,
      o.m_xf
    );
  ...
  k.prototype.Evaluate = function () {
    ...
    this.b2CollidePolyAndEdge(
      ...
      this.m_fixtureA.GetShape() instanceof K ? this.m_fixtureA.GetShape() : null,
      j.m_xf,
      this.m_fixtureB.GetShape() instanceof G ? this.m_fixtureB.GetShape() : null,
      o.m_xf
    );
  };
  ...
  z.prototype.Evaluate = function () {
    ...
    S.CollidePolygons(
      ...
      this.m_fixtureA.GetShape() instanceof K ? this.m_fixtureA.GetShape() : null,
      j.m_xf,
      this.m_fixtureB.GetShape() instanceof K ? this.m_fixtureB.GetShape() : null,
      o.m_xf
    );
  };
...
(function () {
      ...
      for (var M = this.m_bodyList; M; M = M.nextBody) {
        ...
        if (L.IsAwake() != false) {
          ...
          if (!(Y < Number.MIN_VALUE)) {
            ...
            L.ApplyForce(W, I);
            L.ApplyTorque(
              (-L.GetInertia() / L.GetMass()) * Y * L.GetAngularVelocity() * this.angularDrag
            );
          }
        }
      }
  ...
  A.prototype.Step = function (M) {
    ...
    for (var L = this.m_bodyList; L; L = L.nextBody) {
      var I = L.body;
      I.IsAwake() &&
        I.SetLinearVelocity(new K(I.GetLinearVelocity().x + M.x, I.GetLinearVelocity().y + M.y));
    }
  };
    ...
    if (!(M <= Number.MIN_VALUE)) {
      ...
      for (var L = this.m_bodyList; L; L = L.nextBody) {
        ...
        if (I.IsAwake()) {
          ...
          I.SetLinearVelocity(
            new K(I.GetLinearVelocity().x + W.x * M, I.GetLinearVelocity().y + W.y * M)
          );
        }
      }
    }
...
(function () {
  ...
  k.prototype.InitVelocityConstraints = function (d) {
    ...
    if (d.warmStarting) {
      ...
      d =
        this.m_impulse.x * this.m_perp.x + (this.m_motorImpulse + this.m_impulse.y) * this.m_axis.x;
      j =
        this.m_impulse.x * this.m_perp.y + (this.m_motorImpulse + this.m_impulse.y) * this.m_axis.y;
  ...
  E.prototype.InitVelocityConstraints = function (d) {
    ...
    this.m_u1.Set(
      h.m_sweep.c.x + o - (this.m_ground.m_xf.position.x + this.m_groundAnchor1.x),
      h.m_sweep.c.y + q - (this.m_ground.m_xf.position.y + this.m_groundAnchor1.y)
    );
...
(function () {
  ...
  F.prototype._color = function (G, K) {
    return (
      "rgba(" + ((G & 16711680) >> 16) + "," + ((G & 65280) >> 8) + "," + (G & 255) + "," + K + ")"
    );
  };
